<x-layout title="New Expense - {{ $category->name }} - {{ $colocation->name }}">
    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white shadow-xl rounded-xl p-8">
                <h1 class="text-2xl font-bold text-gray-900 mb-2">New Expense</h1>

                <form method="POST"
                    action="{{ route('colocations.categories.expenses.store', [$colocation, $category]) }}">
                    @csrf
                    <div class="space-y-6">
                        {{-- Date --}}
                        <div>
                            ...
                            <select name="payer_id" required
                                ...
                                @foreach ($members as $member)
                                    <option value="{{ $member->id }}"
                                        {{ old('payer_id') == $member->id ? 'selected' : '' }}>
                                        {{ $member->name }}
                                    </option>
                                @endforeach
                            </select>
